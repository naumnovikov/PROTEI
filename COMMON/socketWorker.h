...
class SocketWorker {
 ...
 public:
  ...
  int bindListenerForConnections(const uint16_t port, const char* IP,
                                 int listenerForConnections) const;
  bool sendDistance(int client_sock, uint32_t rest_len, bool& client_ok,
                    const std::vector<float>& positio, const char* client_ip,
                    uint16_t client_port) const;
  int connectAppSocketToServer(int sock, uint16_t server_port,
