...
uint32_t SocketWorker::decodeUint32FromBEBytes(const uint8_t* data) const noexcept {
...
bool SocketWorker::recv_full(int sock, uint8_t* buf, std::size_t len) const noexcept{ 
    while (len > 0) {
...
void SocketWorker::sendResponceToClient(int sock, uint8_t format, float distance) const{
    std::vector<uint8_t> responce;
    if (format == binaryTypeByte) { 
        constexpr uint32_t rest_len{1 + 4};
        responce.reserve(4 + rest_len);

        responce.push_back((rest_len >> 24) & 0xFF);
        responce.push_back((rest_len >> 16) & 0xFF);
        responce.push_back((rest_len >> 8) & 0xFF);
        responce.push_back(rest_len & 0xFF);

        responce.push_back(0x01);

        uint8_t fbuf[4];
        encodeFloatToBEBytes(distance, fbuf);
        responce.insert(responce.end(), fbuf, fbuf + 4);
        ...
        spdlog::error("Unknown protocol format");
        return;
    }
    send(sock, responce.data(), responce.size(), 0);
};

void SocketWorker::push4BytesInBE(std::vector<uint8_t>& msg, uint32_t data) const{ 
...
void SocketWorker::fillMsgInBinaryFormatInBE(std::vector<uint8_t>& msg, const std::vector<float>& location) const{
...
void SocketWorker::fillMsgInJSONFormatInBE(std::vector<uint8_t>& msg, const std::vector<float>& location) const{
...
float SocketWorker::getDistance(int sock, uint32_t rest_len) const{
    ...
    if (format == JSONTypeByte) {
        ...
        try {
            ...
            distanceResult = worker.value("distance", -1.0f); 
...
int SocketWorker::bindListenerForConnections(const uint16_t port, const char* IP, int listenerForConnections) const{
    struct sockaddr_in addr{};
    addr.sin_family = AF_INET;
    addr.sin_port = htons(port);
    addr.sin_addr.s_addr = inet_addr(IP);

    return bind(listenerForConnections, (struct sockaddr*)&addr, sizeof(addr)) < 0;
}

bool SocketWorker::sendDistance(int client_sock, float rest_len, bool& client_ok, const std::vector<float>& position) const{
    std::vector<uint8_t> rest(rest_len);
    ...
    if (format == binaryTypeByte) { 
        if (rest_len != protocol_len + binary_type_data_len) {
            spdlog::error("Binary: expected 12 bytes, got {}", rest_len - 1);
            return false;
        }
        x = decodeFloatFromBEBytes(rest.data() + protocol_len);
        y = decodeFloatFromBEBytes(rest.data() + protocol_len + float_len);
        z = decodeFloatFromBEBytes(rest.data() + protocol_len + float_len + float_len);
        valid = true;
    } else if (format == JSONTypeByte) {
        std::string json_str(rest.begin() + 1, rest.end());
        try {
            json worker{json::parse(json_str)};
            if (worker.contains("location") && worker["location"].is_array() && worker["location"].size() >= 3) {
                x = worker["location"][0].get<float>();
                y = worker["location"][1].get<float>();
                z = worker["location"][2].get<float>();
                valid = true;
            } else {
                spdlog::warn("JSON missing 'location' array");
            }
        } catch (const std::exception& e) {
            spdlog::warn("JSON parse error: {}", e.what());
        }
    } else [[unlikely]]{
        spdlog::warn("Unknown format");
        return false;
    }
    ...
    float distance{static_cast<float>(std::sqrt(std::pow(std::abs(position[0] - x), 2) + 
                                    std::pow(std::abs(position[1] - y), 2) + 
                                    std::pow(std::abs(position[2] - z), 2)))};
    spdlog::info("Received: ({}, {}, {}) -> distance = {}", x, y, z, distance);

    sendResponceToClient(client_sock, format, distance);
    return true;
}

int SocketWorker::connectAppSocketToServer(int sock, uint16_t server_port, const char* server_IP) const noexcept{ 
    struct sockaddr_in addr;
    addr.sin_family = AF_INET;
